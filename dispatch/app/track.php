...
if(isset($_POST['user_check'])){
	...
	$users_get = "SELECT * FROM `deliveries` WHERE `waybill` LIKE '%$user_check%' AND `disp_id` = $uid ORDER BY `rec_state` DESC";
	...
	$uid = $_SESSION['did'];
		$c = '';
		$users_get = "SELECT * FROM `deliveries` WHERE `disp_id` = $uid ORDER BY `id` DESC LIMIT 10";
