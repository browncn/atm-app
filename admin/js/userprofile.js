	...
	function dept_get(){
		send_data = '&deptget=d';
		ajax_data_append('app/userprofile.php', 'dept', send_data);
	}
	...
	function userget(){
		...
		gid('pass').value = dlist[2];
		gid('email').value = dlist[3];
		gid('phone').value = dlist[4];
		gid('gender').value = dlist[5];
		gid('dept').innerHTML = '<option value="' + dlist[6] + '">' + dlist[6] + '</option>' + this.innerHTML;
		function sub_search2(){
			sub_search();
			gid('sub').innerHTML = '<option value="' + dlist[7] + '">' + dlist[7] + '</option>' + gid('sub').innerHTML;
		}
		gid('check').addEventListener('load', sub_search2());
		
		gid('state').value = dlist[8];
		gid('addr').value = dlist[9];
	}
	
	gid('check').addEventListener('load', userget());
	
	function del0() {
		gid('modal-body').innerHTML='<p style="color:red;">Are you sure you want to delete this user?</p>    <p style="color:red;">This cannot be undone...</p>';
