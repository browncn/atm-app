	...
	gid("branchlist").addEventListener('load', c());
	function c(){
		ajax_data('app/branchlist.php', 'branchlist', null);
	}
	
	//usearch_input.input = sel();
	
	//gid('branchlist').addEventListener('onStateChange', sel());
	function sel(){
		user_check = gid('usearch_input').value;
		form_array = '&branch_check=' + user_check;
		send_data = form_array;
		ajax_data('app/branchlist.php', 'branchlist', send_data);		
	}
