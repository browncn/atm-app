	...
	function avail(){
		summary = 'summary';
		sum = '&summary=' + summary;
		ajax_data('app/bt_bus_sel.php', 'summary', sum);
		err = 'err';
		form_array = '&err=' + err;
		send_data = form_array ;
		ajax_data('app/bt_bus_sel.php', 'rides', send_data);
	}
	...
	function bt_bus_sel_cont(){
		car_sel = checky;
		cont = gid('bt_bus_sel_continue').value;
		...
		if(car_sel == ''){
			...
			'&cont=' + cont;
			send_data = form_data;
			ajax_data_red('app/bt_bus_sel.php', 'error', send_data);
		}
	}
