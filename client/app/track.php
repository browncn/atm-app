...
if(isset($_POST['user_check'])){
	...
	$users_get = "SELECT * FROM `deliveries` WHERE `waybill` LIKE '%$user_check%' AND `uid` = $uid ORDER BY `rec_state` DESC";
	...
	if(mysqli_num_rows($users_get_res) == 0){
		...
		while ($pendings_get_row = mysqli_fetch_assoc($users_get_res)){
			...
			echo '<tr>
				...
				<td style="margin-right:20px;color:' . $c . ';padding-top:10px;padding-bottom:10px;">' . $pendings_get_row['deliv_status'] . '</td>
	...
	$uid = $_SESSION['cid'];
		$c = '';
		$users_get = "SELECT * FROM `deliveries` WHERE `uid` = $uid ORDER BY `id` DESC LIMIT 10";
